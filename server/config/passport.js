    ...
    passport.use(new LocalStrategy({
        ...
        userProc.readByEmail(email).then(function(user) {
            ...
            .then(function(matches) {
                if (matches) {
                    // if the password they are using to log in matches the hash in the database after hashing
                    delete user.password;
                    return done(null, user);
                } else {
                    // if the password they are using to log in does not match the hash in the database after the hash
                    return done(null, false, { message: loginError });
                }
            });
        }).catch(function (err) {
